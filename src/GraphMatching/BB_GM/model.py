...
import src.GraphMatching.utils.backbone as backbone
from .affinity_layer import InnerProductWithWeightsAffinity
from .sconv_archs import SiameseSConvOnNodes, SiameseNodeFeaturesToEdgeFeatures
...
from src.GraphMatching.utils.feature_align import feature_align
from src.GraphMatching.utils.utils import lexico_iter
from src.GraphMatching.utils.visualization import easy_visualize
from IPython.core.debugger import Pdb
import torch.nn.functional as Fn
def normalize_over_channels(x):
    channel_norms = torch.norm(x, dim=1, keepdim=True)
    return x / (channel_norms + 1e-4)
...
class Net(backbone.VGG16_bn):
    ...
    def forward(
        ...
        if self.training:
            unary_costs_list = [
                [
                    x + self.margin*gt[:dim_src, :dim_tgt]  # Add margin with alpha = 1.0
        ...
        if cfg.BB_GM.solver_name == "none":
            matchings = [[Fn.one_hot(x.argmax(dim=-1),x.shape[-1]) for x in unary_costs_list[0]]] 
            #return unary_costs_list
        elif cfg.BB_GM.solver_name == "lpmp":
